...
#else
#ifdef _ttmath
    ttmath::UInt<KMER_PRECISION> t = kmer&63;
    t.ToInt(result);
#else
    result = kmer&63;
#endif
...
int code2prot (kmer_type code, char *seq, char *seqR, int sizeKmer, kmer_type kmerMask)
{
    int i;
    kmer_type temp = code;
    char bin2NT[4] = {'A','C','T','G'};
    char bin2P[64] = {'K','N','N','K','T','T','T','T','I','I','I','M','R','S','S','R','Q','H','H','Q','P','P','P','P','L','L','L','L','R','R','R','R','*','Y','Y','*','S','S','S','S','L','F','F','L','*','C','C','W','E','D','D','E','A','A','A','A','V','V','V','V','G','G','G','G'};

    for (i=(sizeKmer-1)/3; i>=0; i--)
    {
        seq[i]=bin2P[first_P(temp&63)];
	//printf("here %c\n",seq[i]);
        temp = temp>>6;
    }
    ...
    for (i=(sizeKmer-1)/3; i>=0; i--)
    {
        seqR[i]=bin2P[first_P(temp&63)];
	//printf("here %c\n",seq[i]);
        temp = temp>>6;
    }

    seq[(sizeKmer-1)/3+1]='\0';
    seqR[(sizeKmer-1)/3+1]='\0';
    return sizeKmer;
}
