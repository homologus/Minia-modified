...
int debloom(int order, int max_memory)
{
        ...
        for(nt=0; nt<4; nt++) 
        {
            ...
            for (strand = 0; strand < 2 ; strand++)
            {
                ...
                if(bloo1->contains(new_graine)){   // extension is positive
                    ...
                    // todo-order>0: code best estimate of parameters in the presence of order>0 deblooming  
    ...
    uint64_t max_kmer_per_part = 0.8*max_memory*1024LL*1024LL /sizeof(cell<kmer_type>);
